...
class PstatManager:
    def __init__(
        self,
        pstat_com: Union[str, GamryCOMObject] = GamryObjects.PC5Pstat,
        timeout: float = 0.5,
    ):
    ...
    def make_pstat(self) -> None:
        """Make a new pstat object, if the manager hasn't already constructed one.
        Will not make a new pstat object if there is already a pstat."""
        if not self.has_pstat():
